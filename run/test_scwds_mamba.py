...
# ==========================================
# Global Metric Config (Aligned with SimVP)
# ==========================================
class MetricConfig:
    MM_MAX = 30.0
...
def plot_seq(obs, tru, pred, path):
    ...
    for t in range(T):
        if t < obs.shape[0]: 
            ax[0,t].imshow(obs[t], cmap='turbo', vmin=0, vmax=1)
            if t==0: ax[0,t].set_ylabel('Obs')
        ax[0,t].axis('off')
        
        ax[1,t].imshow(tru[t], cmap='turbo', vmin=0, vmax=1)
        if t==0: ax[1,t].set_ylabel('GT')
        ax[1,t].axis('off')
        
        ax[2,t].imshow(pred[t], cmap='turbo', vmin=0, vmax=1)
        if t==0: ax[2,t].set_ylabel('Pred')
        ax[2,t].axis('off')
